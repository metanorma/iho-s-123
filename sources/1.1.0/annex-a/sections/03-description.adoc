...
8+| *S-123 [Geo Feature/Information Type]: Feature (S-57 Acronym)*
S-123 feature and corresponding S-57 acronym (if applicable)
8+| *Super-Type:* super-type (clause reference, should be hyperlink when published)
8+| *Sub-Types:* sub-types (clause references, should be hyperlinks when published)
...
2+h| S-123 Attribute 2+h| S-57 Acronym 2+h| Allowable Encoding Value h| Type h| Multiplicity

2+| This section lists the allowable *local* attributes for the S-123 feature or information type.
2+| 2+| | |

8+|

8+| *[underline]#Feature/information associations#* (allowable *local* associations)
...
S-123 Attribute: Indentation of attributes indicates sub-attributes
...
Such bindings are defined in the S-123 Feature Catalogue. The full
list of enumerates that may be assigned to an attribute in S-123 can
...
attributes that can be simple type or complex type (see
<<S123,clause=12.3>>). The prefix (S) indicates that the attribute
...
are indicated by indentation of the attribute name in the S-123 Attribute
column.
...
notations, see <<S100,part=1>>. For explanations of the conventions used
...
these two instances" and there must be an appropriate instance at
both ends. Associations that are not mandatory should be encoded if
